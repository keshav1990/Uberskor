...
class Tt_model extends CI_Model {
	...
	function __construct()
	{
		...
		$this->today_from =  "2017-06-05 00:00:00";
		$this->today_to =  "2017-06-05 23:59:59";
	...
	{
		$this->db->select('e.id,prop1.name as prop,e.name,l.name as country,esl.name AS status_text,IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,e.name as eventname,c.flag,
	...
	*/
	...
	public function get_league_match($tournament_stageId,$sfk,$limit='')
	{
				$leaguesql = 'SELECT
						    ...
						    ts.id AS stage_id,
							IF(lts.name IS NULL or lts.name="", ts.name,lts.name) AS tsname,c.flag,
						    DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
						    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
						    MIN(IF( ep.number = 1, p.name, NULL)) AS home_team,
						    MIN(IF( ep.number = 1, ep.id, NULL)) AS home_id,
	...
	{
		$this->db->select('e.id,c.name as country,c.flag,
	...
	*/
	...
	public function get_current_tournament($sportFK)
	{
		...
		$this->db->where('ts.startdate <= "'.date("Y-m-d 23:59:59").'"');
	...
	*/
	...
	{
		$this->db->select('e.id,prop1.name as prop,e.name,l.name as country,es.name AS status_text,IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,
					MIN(IF( prop1.name = "Live", prop1.value, NULL)) AS tlive,
					MIN(IF( prop1.name = "Cup", prop1.value, NULL)) AS cup,
					MIN(IF( prop1.name = "City", prop1.value, NULL)) AS city,
					MIN(IF( prop2.name = "Live", prop2.value, NULL)) AS elive,
