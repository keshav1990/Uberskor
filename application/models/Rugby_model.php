...
class Rugby_model extends CI_Model {
	...
	*/
	...
	{
				$leaguesql = 'SELECT
						    e.id,c.flag,e.name as eventname,
	...
	public function get_leagues($sportFK)
	{
		...
		$this->db->where_in("ts.name",array("Top 14","Aviva Premiership","Six Nations","World Cup Final","Super Rugby","Rugby Championship","NRL","Super League"));
	...
	{
		$this->db->select('e.id,l.name as country,e.name,c.flag,e.name as eventname,
				    IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,
				    DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
				    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
				    MIN(IF( ep.number = 1, p.name, NULL)) AS home_team,
