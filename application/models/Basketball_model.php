...
class Basketball_model extends CI_Model {
	...
	function __construct()
	{
		...
		//$this->today_to =  "2017-06-19 00:00:00";
		//$this->today_from =  "2017-06-19 23:59:59";
	}
	...
	{
		$this->db->select('e.id,l.name as country,e.name as event_name,e.name as eventname,
				    IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,c.flag,
		...
		$this->db->join('property AS prop', 'e.id = prop.objectFK AND prop.object ="event"', 'inner');
	...
	*/
	public function get_result($where)
	{
		...
		GROUP_CONCAT(IF(r.result_typeFK = 60, r.value, NULL) ) as quarter1,
		GROUP_CONCAT(IF(r.result_typeFK = 61, r.value, NULL)) as quarter2,
		GROUP_CONCAT(IF(r.result_typeFK = 62, r.value, NULL)) as quarter3,
		GROUP_CONCAT(IF(r.result_typeFK = 63, r.value, NULL)) as quarter4',false);
		$this->db->where($where);
	...
	public function get_leagues($sportFK)
	 {
		...
		$this->db->where_in("ts.name",array("NBA","LNB","EURO Basket","Legae","Super League","Euroleague","Liga ABA","Avrupa Ligi"));
		...
		$TODAY =  date("Y-m-d 00:00:00");
				$sql = 'SELECT
					...
					WHERE
						...
						e.del = "no" AND e.startdate <= "'.$this->today_from.'" and 
						p.del = "no" 
					GROUP BY
						e.id
					ORDER BY
	...
	{
		$this->db->select('l.id,lt.name as player_type,l.participantFK,p.name,l.pos,l.shirt_number,l.lineup_typeFK');
