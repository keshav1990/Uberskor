...
class Handball_model extends CI_Model {
	...
	public function get_live_list($sportFK,$date='',$wherein='')
	{
 		$this->db->select('e.id,l.name as country,c.flag,e.name as eventname,
				    IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,e.name as eventname,
	...
	*/
	...
	public function get_league_match($tournament_stageId,$sfk,$limit='')
	{
				$leaguesql = 'SELECT
							...
							ts.name as tsname,c.flag,e.name as eventname,
	...
	public function get_leagues($sportFK)
	{
		...
		$this->db->where_in("ts.name",array("Bundesliga","Lidl Starligue","European Championship - Ladies","Champions League Men","World Championship Finals","Liga ASOBAL"));
	...
	public function match_detail($eventId)
	{
		$matchDetailSql = 'SELECT
					...
					FROM
					    ...
					    property AS prop ON ep.id = prop.objectFK AND prop.object ="event_participants" INNER join
					    property AS prop2 ON e.id = prop2.objectFK AND prop2.object ="event"
					WHERE
					    e.id= '.$eventId.'
					GROUP BY
					    e.id
	...
	public function get_statistics($eid,$epid)
	{
		$this->db->select("sd.value,sd.code,sp.participantFK,st.id as standing_typeFK,st.name as standing_type,p.name as participant");
		...
		$this->db->join('participant AS p', 'p.id = sp.participantFK', 'left');
		$this->db->where("s.object='event_participants' and s.standing_typeFK=4 and s.objectFK=$epid");
