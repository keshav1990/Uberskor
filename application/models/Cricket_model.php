...
class Cricket_model extends CI_Model {
	...
	{
		$this->db->select('e.id,prop1.name as prop,e.name,l.name as country,es.name AS status_text,IF(ls.name IS NULL or ls.name="", ts.name,ls.name) AS stage_name,ts.id AS stage_id,p2.name as toss_winner,c.flag,e.name as eventname,
					MIN(IF( prop1.name = "Live", prop1.value, NULL)) AS tlive,
					MIN(IF( prop1.name = "Cup", prop1.value, NULL)) AS cup,
					MIN(IF( prop1.name = "Ranking", prop1.value, NULL)) AS ranking,
					...
					p2.name as toss_winner,
					MIN(IF( prop2.name = "toss_event_participantFK", prop2.value, NULL)) AS toss_event_participantFK,
				    ');
		...
		$this->db->join('property AS prop2', 'e.id=prop2.objectFK  AND prop2.object ="event" ', 'inner');
		$this->db->join('event_participants AS ep', 'e.id = ep.eventFK', 'INNER');
		$this->db->join('participant AS p', 'ep.participantFK = p.id', 'left');
		$this->db->join('event_participants AS ep2', 'ep2.id = prop2.value and  prop2.object ="event"', 'INNER');
		$this->db->join('participant AS p2', 'ep2.participantFK = p2.id', 'left');
	...
	/* This function is used to get the addtional details of aan events */
	public function get_event_property($where)
	{
		$this->db->select('MIN(IF( prop2.name = "Live", prop2.value, NULL)) AS elive,
					MIN(IF( prop2.name = "Round", prop2.value, NULL)) AS round,
					MIN(IF( prop2.name = "gameType", prop2.value, NULL)) AS gameType,
					MIN(IF( prop2.name = "StartTimeToBeDecided", prop2.value, NULL)) AS startTimeToBeDecided,
					MIN(IF( prop2.name = "toss_choose", prop2.value, NULL)) AS toss_choose,
					MIN(IF( prop2.name = "comment", prop2.value, NULL)) AS comment,
					MIN(IF( prop2.name = "power_play_in_progress", prop2.value, NULL)) AS power_play_in_progress,
					MIN(IF( prop2.name = "power_plays", prop2.value, NULL)) AS power_plays,
							');
		$this->db->where($where);
		$result = $this->db->get("property as prop2");
		return $result->result();
	}
	...
	*/
	public function get_result($where)
	{
		$this->db->select('
					...
					MIN(IF( r.result_code = "overs", r.value ,NULL )) AS overs,
					MIN(IF( r.result_code = "points", r.value ,NULL )) AS runs,
					MIN(IF( r.result_code = "wickets", r.value ,NULL )) AS wickets,
					MIN(IF( r.result_code = "extra", r.value ,NULL )) AS extra,
	...
	{
			$this->db->select('e.id,e.name,e.name as eventname,l.name as country_name,es.name AS status_text,IF(lts.name IS NULL or lts.name="", ts.name,lts.name) AS league_name,ts.id AS stage_id,p2.name as toss_winner,t.name as tournament_name,ls.name as sport_name,
					MIN(IF( prop1.name = "Live", prop1.value, NULL)) AS tlive,c.flag,
					MIN(IF( prop1.name = "Cup", prop1.value, NULL)) AS cup,
					MIN(IF( prop1.name = "Ranking", prop1.value, NULL)) AS ranking,
					DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
				    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
	...
	public function get_programed_match($tsfk,$sportFK,$limit='20')
	{
		...
		$this->db->select('e.id,c.name as country,
				    ts.name AS league_name,ts.id AS stage_id,c.flag,e.name as eventname,
				    DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
				    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
				    MIN(IF( ep.number = 1, p.name, NULL)) AS home_team,
				    MIN(IF(ep.number = 2, p.name, NULL)) AS away_team,
	...
	public function get_leagues($sportFK)
	{
		...
		$this->db->where_in("ts.name",array("IPL","Test Series","ICC Champions Trophy","One Day International","Twenty20 International"));
	...
	public function match_detail($eventId)
	{
		$matchDetailSql = 'SELECT
					    ...
					    ts.countryFK AS cfk,
						l.name as country,
					    ts.id AS stage_id,c.flag,e.name as eventname,
					    DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
					    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
					    MIN(IF( ep.number = 1, p.name, NULL)) AS home_team,
	...
	public function get_scopre_result($epid)
	{
		$this->db->select("ep.id,
						MIN(IF(sr.scope_data_typeFK = 2,sr.value , NULL)) AS points,	
						MIN(IF(sr.scope_data_typeFK = 13,sr.value , NULL)) AS four,	
						MIN(IF(sr.scope_data_typeFK = 14,sr.value , NULL)) AS six,	
						MIN(IF(sr.scope_data_typeFK = 20,sr.value , NULL)) AS wickets,	
						MIN(IF(sr.scope_data_typeFK = 24,sr.value , NULL)) AS overs,	
						MIN(IF(sr.scope_data_typeFK = 256,sr.value , NULL)) AS wickets_won,	
						MIN(IF(sr.scope_data_typeFK = 262,sr.value , NULL)) AS final_outcome,	
						MIN(IF(sr.scope_data_typeFK = 269,sr.value , NULL)) AS top_batsman,	
						MIN(IF(sr.scope_data_typeFK = 270,sr.value , NULL)) AS top_bowler,	
						MIN(IF(sr.scope_data_typeFK = 271,sr.value , NULL)) AS most_match_sixes,	
						MIN(IF(sr.scope_data_typeFK = 272,sr.value , NULL)) AS runs_per_over,	
						");
		$this->db->from('event_participants AS ep');
		$this->db->join('scope_result AS sr', 'sr.event_participantsFK = ep.id', 'INNER');
		$this->db->join('scope_data_type  AS sdt', 'sdt.id = sr.scope_data_typeFK', 'LEFT');
		$this->db->where('ep.id="'.$epid.'"');
		$result = $this->db->get()->result();
		return $result;	
	}
