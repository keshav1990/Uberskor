...
*/
class Snooker_model extends CI_Model {
	
	function __construct()
	{
		parent::__construct();
		/* Setting global stardate and enddate */
		$this->today_from =  "2017-07-08 00:00:00";
		$this->today_to =  "2017-07-08 23:59:59";
	...
	public function get_live_list($sportFK,$date='',$wherein='')
	{
		$this->db->select('e.id,e.name,l.name as country,c.flag,e.name as eventname,
					...
					MIN(IF( prop2.name = "EventTypeName", prop2.value , NULL )) AS event_type,
	...
	*/
	...
	public function get_league_match($tournament_stageId,$sfk,$limit='')
	{
			$this->db->select('e.id,e.name,l.name as country_name,c.flag,e.name as eventname,
					...
					MIN(IF( ep.number = 1, p.name, NULL)) AS home_team,
					MIN(IF( ep.number = 1, ep.id, NULL)) AS home_epid,ls.name as sport_name,DATE_FORMAT(e.startdate, "%m") as diff_month,
				    IF(e.status_type = "notstarted", "-",(MIN(IF(ep.number = 1, r.value, NULL)))) AS home_score,
				    IF(e.status_type = "notstarted", "-",(MIN(IF(ep.number = 2, r.value, NULL)))) AS away_score,
				    MIN(IF(ep.number = 2, p.name, NULL)) AS away_team,
			...
			$this->db->join('language  AS ls', 'ts.id = ls.objectFK and ls.object="sport" and ls.language_typeFK=31', 'left');
	...
	public function get_leagues($sportFK)
	{
		...
		$this->db->where_in("ts.name",array("The Masters","World Championship","UK Championship"));
	...
	public function get_team_against_match($teamId)
	{
				$sql = 'SELECT
					...
					WHERE
						...
						e.del = "no" AND
						p.del = "no" and 
						e.startdate <= "'.date("Y-m-d 00:00:00").'" 
					GROUP BY
						e.id
					ORDER BY
		...
		$TODAY =  date("Y-m-d 00:00:00");
				$sql = 'SELECT
					...
					WHERE
						...
						e.del = "no" AND
						p.del = "no" and 
					e.startdate <= "'.date("Y-m-d 23:59:59").'" 
					GROUP BY
						e.id
					ORDER BY
						e.startdate, e.id limit 5' ;
