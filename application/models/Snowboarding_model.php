...
*/
class Snowboarding_model extends CI_Model {
	
	function __construct()
	{
		parent::__construct();
		/* Setting global stardate and enddate */
		$this->today_from =  "2017-03-4 00:00:00";
		$this->today_to =  "2017-03-4 23:59:59";
	...
	public function get_live_list($sportFK,$date = '',$wherein='',$tsid = '')
	{
		$this->db->select('e.id,e.name,l.name as country,e.name as eventname,c.flag,
					...
					MIN(IF( prop.name = "discipline", prop.value , NULL )) AS discipline,	
					MIN(IF( prop.name = "GroupName", prop.value , NULL )) AS groupName,
					');
	...
	*/
	...
	{
			$this->db->select('
				p.id as racer_id,p.name as racer_name,c.name as country,ep.id as epid,c.flag,
				...
				MIN(IF(r.result_code="comment",r.value,NULL)) as comment
						');
			...
			//$this->db->join('scope_result   AS sr', 'sr.event_participantsFK  = ep.id ', 'left');
			...
			$this->db->where("ep.eventFK = $event ");
	...
	public function get_league_match($tournament_ttId,$sfk,$limit='')
	{
				$this->db->select('e.id,e.name,l.name as country_name,e.name as eventname,c.flag,
				    ...
				    DATE_FORMAT(e.startdate, "%d.%m.%Y") AS startdate,
				    DATE_FORMAT(e.startdate, "%H:%i") AS starttime,
					es.name AS status_text,t.name as tournament_name,				
					MIN(IF( prop.name = "Live", prop.value , NULL )) AS elive,	
					MIN(IF( prop.name = "Round", prop.value , NULL )) AS round,	
					MIN(IF( prop.name = "Type", prop.value , NULL )) AS type,	
					MIN(IF( prop.name = "discipline", prop.value , NULL )) AS discipline,	
		...
		$this->db->join('country  AS c', 'c.id  = ts.countryFK', 'left');
		$this->db->join('property as prop', 'prop.objectFK  = e.id and prop.object = "event" ', 'left');
		$this->db->join('sport AS s', 'prop.objectFK  = e.id and prop.object = "event" ', 'left');
		$this->db->join('language  AS l', 'l.objectFK  = c.id and l.object="country" and l.language_typeFK=31', 'left');
		...
		$this->db->WHERE('ts.id ="'.$tournament_ttId.'" ');
		$this->db->WHERE('e.del ="no"');
	...
	/*  @Description : This function is to get program matches which is going to be play in a particular league. */
	public function get_programed_match($tsfk,$sportFK,$limit='')
	{
	...
	public function get_current_races($sportFK)
	{
		...
		$this->db->select("ts.id,ts.name,e.name as event,e.id as eventid,c.name as country_name,c.flag");
		...
		//$this->db->where('e.startdate between "'.$this->today_from.'" and "'.$this->today_to.'" ');
		$this->db->where(array('ts.enddate >='=>$this->today_to,'ts.startdate >= '=>'2017-01-01 00:00:00'));
		$this->db->where('e.del = "no" ');
		$this->db->group_by('ts.name');
		$result = $this->db->get()->result();
		return $result;	
