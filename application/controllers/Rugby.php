...
*/
class Rugby extends MY_Controller {
	...
	public function __construct()
	{
		...
		if($this->uri->segment('1')=='ragbi'){
			$this->sportFK = 29; //
		}
		if($this->uri->segment('1')=='rugby-ligi'){
			$this->sportFK = 80; //
		}
	} 
	/* 
	...
	public function getContent()
	{
		if (!$this->input->is_ajax_request()) 
		{
	     //show_error("No direct access allowed");
			...
			$data["game"] = $this->uri->segment('1');
			
			if (!$data["list"] = $this->cache->file->get($data["game"].'cache_key'.$this->uri->segment('1').$data["date"])) {
			  $data["list"] = $this->Rugby_model->get_live_list($this->sportFK,$date);
			  // Save into the cache for 2 minutes
			  $this->cache->file->save($data["game"].'cache_key'.$this->uri->segment('1').$data["date"], $data["list"], 240);
			}
			if (!$theHTMLResponse = $this->cache->file->get(__FUNCTION__ . __CLASS__.$this->uri->segment('1').$date)) {
				...
				$theHTMLResponse    = $this->load->view("rugby/rugby_content", $data, TRUE);
			 	$this->cache->file->save(__FUNCTION__ . __CLASS__.$this->uri->segment('1').$date, $theHTMLResponse, 240);
			}
	...
	public function showLiveEvent()
	{
		if (!$this->input->is_ajax_request()) 
		{
	     show_error("No direct access allowed");
			...
			$data["date"] = $date = $this->Common_games_model->filterval($this->input->post('date',true));
				...
				if($this->uri->segment(4)){
						...
						switch($uri){
							...
							$temptournament_stageId = $this->getNextStageId($tournament_stageName,$country,$this->sportFK);
							  ...
							  {
								$data["programMatch"] = $this->Rugby_model->get_programed_match($tournament_stageId,$this->sportFK,"200");
	...
	*/
	public function common_list(){
		...
		$this->db->cache_off();
		$data["leagues"] = $this->Rugby_model->get_leagues($this->sportFK);
	...
	*/
	...
	public function iframe()
	{
		...
		if (!$theHTMLResponse = $this->cache->file->get(__FUNCTION__ . __CLASS__ .'league_popup'. $data["game"].$this->uri->uri_string())) 
		{	
				if($this->uri->segment(4)){
						$uri = $this->uri->segment(4);
						$data["tlist"] = $data["list"] = $this->Rugby_model->get_league_match($tournament_stageId,$this->sportFK,"limit 1");
			...
			$theHTMLResponse = $this->load->view("rugby/rugby_leaguematch_iframe", $data, TRUE);
				$this->cache->file->save(__FUNCTION__ . __CLASS__ .'league_popup'.$data["game"].$this->uri->uri_string(), $theHTMLResponse, 14400);
		} 
