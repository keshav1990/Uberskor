...
#def-form-table td input, textarea { padding: 4px 4px; margin-bottom: 8px;border-radius: 2px; }	 #def-form-table td textarea {resize: none; }	 #def-form-table td select  { width:159px;padding: 4px 4px;margin-bottom: 8px;border-radius: 2px;}
   ...
   /*for footer color and font color*/
	...
	<?php
	$this->load->view("snooker/sidebar");
	?>
	 ...
	 $.getJSON(url , {date:date}, function( data ) {
		...
		$.each(data,function(e,v){
			...
			var pid = $(".<?=$game?>_event_"+v.id).attr("id");
				...
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_ad time \"><span class=\"Color_anited\"> <b></b><\/span>"+v.starttime+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  rowspan=\"2\"  colspan=\"2\" class=\"cell_aa timer\"><span>"+v.status_text+"<\/span><\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"   class=\"cell_ab team-home ";
				...
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"   class=\"cell_ab score-home ";
				if(v.home_score > v.away_score){
					trHtml1 += "bold ";
				}
				trHtml1 +=  "\" >";
				...
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"   class=\"cell_sd part-bottom\">"+isNull(v.home_result[0].halftime)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_se part-bottom\">"+isNull(v.home_result[0].runningscore)-isNull(v.home_result[0].halftime)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_sf part-bottom\">";
				if(v.home_score > v.home_result[0].ordinarytime){
					trHtml1 += isNull(v.home_result[0].ordinarytime)
				}else{
					trHtml1 += "";
				}
				trHtml1 += "<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  rowspan=\"2\" class=\"cell_oq comparison\">";
				trHtml1 += "<span style=\"\" class=\"live-centre-inr\"><b></b><\/span>";
				trHtml1 += "<\/td>";
				...
				var trHtml2="";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"   class=\"cell_sc team-away ";
				if(v.home_score < v.away_score){
					trHtml2 += "bold ";
				}
				trHtml2 +=  "\" >";
				...
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"   class=\"cell_ab score-away ";
				if(v.home_score < v.away_score){
					trHtml2 += "bold ";
				}
				trHtml2 +=  "\" >";
				...
				trHtml2 += "<\/td>";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_sd part-bottom\">"+isNull(v.away_result[0].halftime)+"<\/td>";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_se part-bottom\">"+isNull(v.away_result[0].runningscore)-isNull(v.away_result[0].halftime)+"<\/td>";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  class=\"cell_se part-bottom\">";
				if(v.away_score > v.away_result[0].ordinarytime){
					trHtml2 += isNull(v.away_result[0].ordinarytime)
				}else{
					trHtml2 += "";
				}
				trHtml2 += "<\/td>  ";
