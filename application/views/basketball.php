                     ...
                     <div class="mbox0px l-brd flag_side">
                        <ul class="menu country-list my-leagues" title="">
                           ...
                           <ul id="my-leagues-list" class="menu" >
							  ...
							  <span class="toggleMyLeague active "></span><a title="<?php echo ucwords($row->country_name) . ': ' . ucwords($row->name) ?>" href="<?php echo base_url($game . '/') . $countryURI . '/' . $URL ?>"><?= $row->name ?></a></li>
                              <?php } ?>
                       <!-- - - -Irame BOx- - - - - -->
	 ...
	 $.getJSON(url , {date:date}, function( data ) {
		...
		$.each(data,function(e,v){
			...
			var pid = $(".<?=$game?>_event_"+v.id).attr("id");
				...
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" rowspan=\"2\" class=\"cell_ad time \"><span class=\"Color_anited\"> <b></b><\/span>"+v.starttime+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" rowspan=\"2\" class=\"cell_aa timer\"><span>"+v.status_text+"<\/span><\/td>";
				trHtml1 += " <td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_ab team-home  \" title=\" \"><span class=\"padl ";
				if(v.home_score > v.away_score){
						trHtml1 += "bold ";
				}
				trHtml1 +=  "\" >";
				trHtml1 += v.home_team+"<span style=\"display:none;\" class=\"mobile_only_score1\">"+isNull(v.home_score)+"</span><\/span><\/td>";
				trHtml1 += "";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_sc score-home ";
				if(v.home_score > v.away_score){
						trHtml1 += "bold ";
				}
				trHtml1 +=  "\" >";
				trHtml1 += isNull(v.home_score)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_sd part-bottom\">"+isNull(v.home_result[0].quarter1)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_se part-bottom\">"+isNull(v.home_result[0].quarter2)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_sf part-bottom\">"+isNull(v.home_result[0].quarter3)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_sg part-bottom\">"+isNull(v.home_result[0].quarter4)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_sh part-bottom\">&nbsp;<\/td> ";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" colspan=\"1\" rowspan=\"2\" class=\"cell_ia icons\">";
				trHtml1 += "<span class=\"live-centre-inr\"><b><\/b><\/span>";
				trHtml1 += "";
				trHtml1 += "<\/td>";
			
				var trHtml2="";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\" class=\"cell_ac team-away\" title=\"\"><span class=\"padl ";
				if(v.home_score < v.away_score){
						trHtml2 += "bold ";
				}
				trHtml2 +=  "\" >";
				trHtml2 += v.away_team+"<span style=\"display:none;\" class=\"mobile_only_score2\">"+isNull(v.away_score)+"</span><\/span><\/td>";
				trHtml2 += "";
				trHtml2 += "<td  colspan=\"1\" class=\"cell_ta score-away ";
				if(v.home_score > v.away_score){
						trHtml1 += " bold ";
				}
				trHtml2 +=  "\" >";
				trHtml2 += 	isNull(v.away_score)+"<\/td>";
				trHtml2 += "";
				trHtml2 += "<td  onclick=\"detailPopup('"+v.id+"')\" colspan=\"1\" class=\"cell_tb part-top\">"+isNull(v.away_result[0].quarter1)+"<\/td>";
				trHtml2 += "";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_tc part-top\">"+isNull(v.away_result[0].quarter2)+"<\/td>";
				trHtml2 += "";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_td part-top\">"+isNull(v.away_result[0].quarter3)+"<\/td>";
				trHtml2 += "";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_td part-top\">"+isNull(v.away_result[0].quarter4)+"<\/td>";
