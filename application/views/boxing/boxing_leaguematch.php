            							  ...
            							  <div class="table_takim">
												  ...
												  <thead class="no-border-bottom">
													...
													<tr title="<?=$val->eventname?>" onclick='detailPopup("<?=$val->id?>");'  class="<?=$class?> stage-finished cursor child_match_day_<?=($j-1)?>" style="display:none;" >
													   ...
													   <td class="cell_ad time" title=""><?=$val->startdate?> <?=$val->starttime?></td>
													   <td class="cell_ab team-home <?php if($val->winner=="Home"){echo "bold";} ?>" title=""><span class="padr"><?=$val->home_team?></span></td>
													   <td class="cell_ac team-away <?php if($val->winner=="Away"){echo "bold";} ?>" title=""><span class="padl"><?=$val->away_team?></span></td>
													   <td class="cell_sa score  bold" title="">
													   <?php if($val->comment=='TKO' || $val->comment=='W.O.'){ echo $val->comment;
													   }else{
													   echo $val->home_score ." :  " .$val->away_score ;
													   }
													   ?>
													   </td>
													</tr>
													...
													<td colspan="5" style="text-align:center:"><a href="<?=$pathUrl.'/sonuclar'?>" style="color:#fff!important;" onclick="">Daha fazla karsilasma göster</a></td>  
													</tr>
													</tbody>  
												  </table>
            							  </div>
								...
								<div class="table_takim">
									  ...
									  <thead class="no-border-bottom">
										...
										<tr onclick='detailPopup("<?=$val->id?>");' title="<?=$val->eventname?>"  class="<?=$class?> stage-finished cursor child_match_day_<?=($j-1)?>" style="display:none;" >
										   ...
										   <td class="cell_ab team-home <?php if($val->winner=="Home"){echo "bold";} ?>" title=""><span class="padr"><?=$val->home_team?></span></td>
										   <td class="cell_ac team-away <?php if($val->winner=="Away"){echo "bold";} ?>" title=""><span class="padl"><?=$val->away_team?></span></td>
										   <td class="cell_sa score  bold" title=""> <?php if($val->comment=='TKO' || $val->comment=='W.O.'){ echo $val->comment;
													   }else{
													   echo $val->home_score ." :  " .$val->away_score ;
													   }
													   ?></td>
										...
										<?php $i++; $diff_month = $val->diff_month;  endforeach ?>
										<tr class="event_round">
										<td colspan="5" style="text-align:center:"><a href="javascript:void(0)" style="color:#fff!important;" onclick="loadMore()">Daha fazla karsilasma göster</a></td>  
										</tr>
										</tbody>  
									  </table>
            					</div>
                     ...
                     <div class="mbox0px l-brd flag_side">
                        <ul class="menu country-list my-leagues" title="">
                           ...
                           <ul id="my-leagues-list" class="menu" >
                             ...
                             <li>  <span class="flag flag-<?=strtolower($ct->flag)?>"></span> <a href="<?php echo base_url().$game.'/'.url_title($ct->country,'dash',true).'/'.url_title($ct->category,'dash',true).'+'.url_title($ct->gender,'dash',true)?>"><?=$ct->category.'-'.$ct->gender?></a></li>
                              <?php } }?>
                       <!-- - - -Irame BOx- - - - - -->
