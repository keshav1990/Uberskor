	  ...
	  .display_none{display:none;}
         ...
         <div class="content soccer_inner">
            ...
            <div id="main">
               ...
               <div id="tc">
               <div id="lc">
                     <div class="mbox0px l-brd flag_side mobile-bx-1">
...
function showLiveEvent()
{
	 ...
	 game = "<?php echo $this->uri->segment('1')?>";
	 tdHtml = '';
	 $.getJSON(url , {date:date,game:game}, function( data ) {
		jQuery.parseJSON(JSON.stringify(data));
		$.each(data,function(e,v){
			/* getting parent tr  id */
			var pid = $(".<?=$game?>_event_"+v.id).attr("id");
				var trHtml1="";
				trHtml1 += " <td rowspan=\"2\" class=\"cell_ib icons left\">    <input type=\"checkbox\" onchange='mygames(\"child_"+v.id+","+pid+"\");'  class=\"checkbox parent_"+pid+" child_"+v.id+"\" ><\/td>";
				...
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" colspan=\"6\" class=\"cell_ab team-home ";
				if(v.home_score > v.away_score){
					trHtml1 += "bold ";
				}
				trHtml1 +=  "\" ><span class=\"padl\">";
				trHtml1 += v.home_team;
				trHtml1 += "<span style=\"display:none;\" class=\"mobile_only_score1\">"+isNull(v.home_score)+"</span><\/span><\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" colspan=\"1\" class=\"cell_sc score-home ";
				if(v.home_score > v.away_score){
					trHtml1 += "bold ";
				}
				trHtml1 +=  "\" >";
				trHtml1 += isNull(v.home_score)+"<\/td>";
				trHtml1 += " <td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_sd part-bottom\">"+isNull(v.home_result[0].halftime)+"";
				trHtml1 += "<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_se part-bottom\">"+isNull(v.home_result[0].runningscore-v.home_result[0].halftime)+"<\/td>";
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_sf part-bottom\">"
				if(v.home_score > v.home_result[0].ordinarytime){
				trHtml1 += v.home_result[0].ordinarytime;
				}else{
					trHtml1 += 	"";
				}
				trHtml1 += "<\/td>";
				
				trHtml1 += "<td onclick=\"detailPopup('"+v.id+"')\" rowspan=\"2\" class=\"cell_oq comparison\">";
				...
				var trHtml2="";
				trHtml2 += " <td  onclick=\"detailPopup('"+v.id+"')\" colspan=\"6\" class=\"cell_ac team-away ";
				if(v.home_score < v.away_score){
					trHtml2 += "bold ";
				}
				trHtml2 +=  "\" >";
				trHtml2 += v.away_team;
				trHtml2 += "<span style=\"display:none;\" class=\"mobile_only_score1\">"+isNull(v.away_score)+"</span><\/td>";
				...
				trHtml2 += " <td onclick=\"detailPopup('"+v.id+"')\" colspan=\"1\" class=\"cell_sd part-bottom\">"+isNull(v.away_result[0].halftime)+"";
				trHtml2 += "<\/td>";
				trHtml2 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"1\" class=\"cell_se part-bottom\">"+isNull(v.away_result[0].runningscore - v.away_result[0].halftime)+"<\/td>";
				trHtml2 += "<td  onclick=\"detailPopup('"+v.id+"')\" colspan=\"1\" class=\"cell_sf part-bottom\">"
				if(v.away_score > v.away_result[0].ordinarytime){
				trHtml2 += isNull(v.home_result[0].ordinarytime);
				}else{
					trHtml2 += 	"";
				}
				trHtml2 += "<\/td>";
