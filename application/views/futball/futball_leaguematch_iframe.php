				   ...
				   <div class="team-header">
					...
					<div class="team-name"><?=$tlist[0]->league_name?></div>
                     ...
                     <div id="fsbody" class="flashscore">
                        <div id="fsi"></div>
                        <div class="ifme_content" id="fscon">
						<?php
							$pathUrl = base_url().$game.'/'.$this->uri->segment('3').'/'.$this->uri->segment('4');
						?>
                           <ul class="ifmenu live-menu">
                              <li class="<?php if($this->uri->segment('5') == 'puan-durumu' ){echo "selected";}?>" ><a href="javascript:void(0)" onclick="goTo('puan durumu')">Puan Durumu</a></li>
                           </ul>     
							<!-----Tab Content-4---->
							 <div id="tab-4"  style="<?php if($this->uri->segment('5') == 'puan-durumu' ){ echo 'display:block;';}else{ echo 'display:none;';} ?>" class="fs-table tab-content">
							  ...
							  <tbody>
							  <?php $mkk = 0; foreach($league_standing as $lst ):  $mkk++;?>
								<tr class="glib-participant-nVp0wiqd odd" data-def-order="0" title="">
							   <td class="rank col_rank no q1 col_sorted" title="Kupaya katilim - Sampiyonlar Ligi (Grup Asamasi)"><?=$mkk?>.</td>
