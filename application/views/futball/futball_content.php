                        ...
                        <div id="fscon">
                           ...
                           <div id="tab-1" class="fs-table tab-content" style=" display:block;">
                              ...
                              <div class="odds-content">
                                  ...
                                  //print_r(count($listNew1));
                                  //echo '<pre>';print_r($listNew1);
                                  ///tow loops are used to make the listing acc to stage and than get stage lists
                                  /* foreach($countryListCenter as $c) :
                                  ...
                                  //$listNewN =  $listNew1 ;
                                  //echo '<pre> Hello';
                                  //print_r($centerleagues);
                                  //exit;
                                    if (!empty($centerleagues)) {
                                      $counters = 0 ;
                                      foreach ($centerleagues as $league1) :

                                      ///this is to check if country exist or not
                                        if (!array_key_exists($league1->country, $listNew1)) {
                                          continue ;
                                        }
                                        $listNewN = $listNew1[$league1->country];
                                        $league1->country_name = $league1->country;
                                        $league1->name = $league1->stage_name;
                                  //  echo $c->name.$league1->name;
                                  // print_r($listNewN[$c->name.$league1->name]);
                                  ///this is to check if leauge exist or not
                                   if (!array_key_exists($league1->country . $league1->stage_name, $listNewN)) {
                                          continue ;
                                        }
                                  //print_r($list1);
                                  // foreach ($listNewN as $cou => $list1) :
                                        $list1 = $listNewN[$league1->country . $league1->stage_name];
                                        $listN1 = str_replace(" ", "", $league1->country . $league1->stage_name) ;
                                      //  print_r($list1);
                                        $counters++;
                                  ...
                                  //   print_r($row1);
                                        ...
                                        <span style="" class="Color_anited">
                                         <b></b>
                                                 ...
                                                 <?php
                                                 $liveText = ob_get_clean() ;
                                               }
                                               $parentID = $row1->id ;
                                               if (in_array($parentID, $duplicateHandler) || in_array($league1->country_name . $league1->name, $duplicateHandler)) {
                                                 continue ;
                                               }
                                       ///this is to check name if name is not same from previous than create new one
                                               if ($oldName1 != $league1->country_name . $league1->name) {
                                                 ob_start() ;
                                       // print_r($row1);
                                                 $oldName1 = $league1->country_name . $league1->name ;
                                                 $oldNameCount++;
         ...
         <table class="soccer maintable mobile-vr odds {str_class} pushLeague_in<?= $listN1 ?>   LeaguePushID_in" colgroup="" id="{parent}_show<?= $listN1 ?>" width="100%">
                        ...
                        <input title="Oyunlarima ekle" type="checkbox" onchange='mygames(this,"parent_<?= $listN1 ?>")' id="parent_<?= $listN1 ?>" class="checkbox-custom parent" >
                              ...
                              <span data-flag="<?= $row1->flag ?>" data-country='<?= $league1->country_name ?>' data-stage_id='<?= $row1->stage_id ?>' data-stage_name='<?= $league1->urlname ?>' data-parentid='<?= $listN1 ?>'  onclick="pushLeague(this)" title="
Liglerime ekle" class="{startclass} showme<?= $parentID ?> rshowme<?= $parentID ?> toggleMyLeague 1_3_EwJVdqzn"  ></span>
                                ...
                                <span class="leagu_tble" style="float:right" onclick="openLeague('<?= $url ?>')" title="Puan durumu">LeagueTable</span></div>
                ...
                <input title="Oyunlarima ekle" type="checkbox" value="<?= $row1->id ?>" onchange='mygames(this,"child_<?= $row1->id ?>,<?= $listN1 ?>")'  class="checkbox-custom checkbox parent_<?= $listN1 ?> child_<?= $row1->id ?>" >
                                      ...
                                      <?php echo $liveText ; ?>
                                       ...
                                       <td class="live_lst" onclick='detailPopup("<?= $row1->id ?>");'>
                                        ...
                                        ///this is used if it's last column
                                                if ($k == (count($list1) - 1)) {
                                                  $duplicateHandler[] = $league1->country_name . $league1->name ;
