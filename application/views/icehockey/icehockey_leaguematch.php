...
<style>    #def-form-table td input, textarea { padding: 4px 4px; margin-bottom: 8px;border-radius: 2px; }    #def-form-table td textarea {resize: none; }    #def-form-table td select  { width:159px;padding: 4px 4px;margin-bottom: 8px;border-radius: 2px;}
 ...
 .tab-content {margin-top: 1px;}
				   ...
				   <div class="team-header">
					...
					<div class="team-name"><?=$tlist[0]->league_name?></div>
                     ...
                     <div id="fsbody" class="flashscore">
                        <div id="fsi"></div>
                        <div id="fscon">
						<span class="selected-mob">tüm</span>
						<?php
						...
						?>
                           <ul class="ifmenu live-menu">
							  ...
							  <li class="<?php if($this->uri->segment('4') == 'fikstur' ){echo "selected";}?>" ><a href="javascript:void(0)" onclick="goTo('fikstur')" ><?=mb_convert_encoding("Fikstür", "UTF-8");?></a></li>
                           </ul>
            							  ...
            							  <div class="table_takim">
												  ...
												  <thead class="no-border-bottom">
													   <tr class="league l_1_ljIBgFCg primary-top">
														  ...
														  <span class="flag flag-<?=strtolower($list[0]->flag)?>" title="Almanya"></span>
													...
													<?php $j++; } ?>
													<tr onclick='detailPopup("<?=$val->id?>");' title="<?=$val->eventname?>" class="<?=$class?> stage-finished cursor child_match_day_<?=($j-1)?>" style="display:none;" >
													   <td class="cell_ib icons left" ></td>
													   <td class="cell_ad time" title="<?=$val->eventname?>"><?=$val->startdate?> <?=$val->starttime?></td>
													   <td class="cell_ab team-home <?php if($val->home_score > $val->away_score){echo "bold";} ?>" title="<?=$val->eventname?>" ><span class="padr"><?=$val->home_team?></span></td>
													   <td class="cell_ac team-away <?php if($val->home_score < $val->away_score){echo "bold";} ?>" title="<?=$val->eventname?>"><span class="padl"><?=$val->away_team?></span></td>
													   <td class="cell_sa score  bold" title="<?=$val->eventname?>"><?=$val->home_score?>&nbsp;:&nbsp;<?=$val->away_score?></td>
													</tr>
							 ...
							 <?php if(empty($list)){
								...
								<div class="table_takim">
									  ...
									  <thead class="no-border-bottom">
										...
										<?php $j++; } ?>
										<tr title="<?=$val->eventname?>" onclick='detailPopup("<?=$val->id?>");' title="<?=$val->eventname?>"  class="<?=$class?> stage-finished cursor child_match_day_<?=($j-1)?>" style="display:none;" >
										   <td class="cell_ib icons left" ></td>
   ...
   <div id="tc">
                     ...
                     <div class="mbox0px l-brd">
                        <ul class="menu country-list my-leagues">
                           ...
                           <ul id="my-leagues-list2" class="menu">
                              ...
                              <li> <span class=" flag flag-<?=strtolower($row1->flag)?>"></span><a href="<?php echo base_url().$game.'/'.url_title($row1->country_name,"dash",true).'/'.str_replace("eror",'.',url_title(str_replace(".",'eror',$row1->name),"dash",true))?>"><?=$row1->name?></a></li>
                     ...
                     <div class="mbox0px l-brd">
                        <ul class="menu country-list my-leagues" title="">
                           ...
                           <ul id="my-leagues-list" class="menu" >
                              ...
                              <li style="display:none;"><span class=" flag flag-<?=strtolower($row->flag)?>"></span><span class="toggleMyLeague active "   ></span><a title="<?php echo ucwords($row->country_name) ?>" href="<?php echo base_url($game.'/').url_title($row->country_name,"dash",true).'/'.str_replace("eror",'.',url_title(str_replace(".",'eror',$row->name),"dash",true))?>"><?=$row->name?></a></li>
                              <?php } ?>
                       <!-- - - -Irame BOx- - - - - -->
    ...
    <?php $this->load->view("icehockey/sidebar"); ?>            
