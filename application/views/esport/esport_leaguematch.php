...
<style>    #def-form-table td input, textarea { padding: 4px 4px; margin-bottom: 8px;border-radius: 2px; }    #def-form-table td textarea {resize: none; }    #def-form-table td select  { width:159px;padding: 4px 4px;margin-bottom: 8px;border-radius: 2px;}
   ...
   /*for footer color and font color*/
               ...
               <div id="tc-main">
                 ...
                 <div id="mc" class="sport_page">
				   ...
				   <?=$tlist[0]->sport_name?> &raquo;
				    <a href="#"><span class="flag flag-<?=strtolower($tlist[0]->flag)?>" title="<?=$tlist[0]->country_name?>"></span> <?=$tlist[0]->country_name?>  </a> &raquo;  <a href="#"> <?=$tlist[0]->league_name?> </a> &raquo;  <?=$tlist[0]->tournament_name?>
				   </h2>
                              ...
                              <div class="odds-content">
            							  ...
            							  <div class="table_takim">
												  ...
												  <thead class="no-border-bottom">
													...
													<?php $j++; } ?>
													<tr  onclick='detailPopup("<?=$val->id ?>");' id="" class="<?=$class?> stage-finished cursor child_match_day_<?=($j-1)?>" style="display:none;" title="<?=$val->eventname?>">
													   <td class="cell_ib icons left" ></td>
													   <td class="cell_ad time" ><?=$val->startdate?> <?=$val->startdate?></td>
							 ...
							 <?php if(empty($list)){
								...
								<div class="table_takim">
									...
									<tbody>
										...
										<?php if($diff_month != $val->diff_month || $diff_month == ''){ ?>
										<tr class="event_round darkbg parent_match_day_<?=$j?>" style="display:none;"  >
										   <td colspan="5" style="text-align:left"><span id="counter_<?=$i?>" class="counter"></span>.  <?php echo mb_convert_encoding("Maç Günü", "UTF-8");?></td>
										</tr>
										<?php $j++; } ?>
							  ...
							  <table class="soccer od_table_mx programlar_bg">
								...
								<thead class="no-border-bottom" title="">
								   <tr class="league l_1_ljIBgFCg primary-top" title="">
									  ...
									  <td colspan="3" class="head_ab " title=""> <span class="stats-link link-tables"><span class="stats" title="Puan Durumu">Puan Durumu</span></span><span class="country left"><span class="left flag flag-<?=strtolower($programMatch[0]->flag)?>" title="<?=$programMatch[0]->country?>"></span><span class="name"><span class="tournament_part"><?=$programMatch[0]->league_name?></span></span></span></td>
								   </tr>
								</thead>
								<tbody>
                                    ...
                                    <tr title="<?=$val2->eventname?>" class="<?=$class?> stage-finished" style="cursor: pointer;">
                                       <td class="cell_ib icons left" title=""></td>
                                       <td class="cell_ad time" ><?=$val2->startdate?> ,<?=$val2->startdate?></td>
    ...
    <?php $this->load->view("esport/sidebar"); ?>            
