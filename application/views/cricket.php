...
#def-form-table td input, textarea { padding: 4px 4px; margin-bottom: 8px;border-radius: 2px; }	 #def-form-table td textarea {resize: none; }	 #def-form-table td select  { width:159px;padding: 4px 4px;margin-bottom: 8px;border-radius: 2px;}
   ...
   /*for footer color and font color*/
	...
	<?php
	$this->load->view("cricket/sidebar");
	?>
...
function showLiveEvent()
{
	 ...
	 $.getJSON(url , {date:date}, function( data ) {
		...
		$.each(data,function(e,v){
				...
				var strVar="";
				strVar += " <td rowspan=\"2\" colspan=\"1\" class=\"cell_ib icons left\">    <input type=\"checkbox\" onchange='mygames(\"child_"+v.id+","+pid+"\");'  class=\"checkbox parent_"+pid+" child_"+v.id+"\" ><\/td>";
				strVar += "";
				strVar += "<td onclick=\"detailPopup('"+v.id+"')\" rowspan=\"2\"  colspan=\"2\" class=\"cell_ad time\">"+v.starttime+"<\/td>";
				strVar += "";
				strVar += "<td onclick=\"detailPopup('"+v.id+"')\"  rowspan=\"2\"  colspan=\"2\"  class=\"cell_aa timer\"><span>"+v.status_text+"<\/span><\/td>";
				strVar += "";
				strVar += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"5\" class=\"cell_ab team-home  \" title=\"\"><span class=\"padl ";
				if(v.home_result[0].finalresult =='won'){
					strVar += " bold ";
				}
				strVar +=  "\" >";
				strVar += v.home_team;
				strVar += "<\/span><\/td>";
				strVar += "";
				strVar += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"4\" class=\"cell_sc score-home ";
				if(v.home_result[0].finalresult =='won'){
					strVar += "bold ";
				}
				strVar += "";
				strVar += "\" >";
				strVar += isNull(v.home_score)+"\/"+isNull(v.home_result[0].wickets)+" ("+isNull(v.home_result[0].overs)+")" ;
				strVar += "<\/td>";
				strVar += "";
				strVar += "<td onclick=\"detailPopup('"+v.id+"')\" rowspan=\"2\" colspan=\"2\" class=\"cell_oq comparison\">";
				strVar += "<span style=\"\" class=\"live-centre-inr\"><\/span>";
				strVar += "<\/td>";
				
				
				
				var strVar2="";
				strVar2 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"5\" class=\"cell_ac team-away\" title=\"\"><span class=\"padl ";
				if(v.away_result[0].finalresult =='won'){
					strVar2 += " bold ";
				}
				strVar2 +=  "\" >";
				strVar2 += v.away_team;
				strVar2 += "<\/span><\/td>";
				strVar2 += "";
				strVar2 += "<td onclick=\"detailPopup('"+v.id+"')\" colspan=\"4\" class=\"cell_ta score-away ";
				if(v.away_result[0].finalresult =='won'){
					strVar2 += "bold ";
				}
				strVar2 += "";
				strVar2 += "\" >";
				strVar2 += isNull(v.away_score)+"\/"+isNull(v.away_result[0].wickets)+" ("+isNull(v.away_result[0].overs)+")" ;
				strVar2 += "<\/td>";
					
				var strVar3="";
				strVar3 += "<td onclick=\"detailPopup('"+v.id+"')\"  colspan=\"16\" class=\"cell_ac team-away\" title=\"\"><span class=\"padl\">"+v.comment+"<\/span><\/td>";

			
								
		//trHtml1 = "<?php str_replace("null",'@','"trHtml1+"')?>";
		$(".<?=$game?>_event_"+v.id+"_1").html(strVar);
		$(".<?=$game?>_event_"+v.id+"_2").html(strVar2);
		$(".<?=$game?>_event_"+v.id+"_3").html(strVar3);
